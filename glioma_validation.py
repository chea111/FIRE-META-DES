# External validation on Glioma dataset
...
# Load data
df = pd.read_csv('Glioma.csv', encoding='gbk', index_col=0)
...
X_train_main, X_test_main, y_train_main, y_test_main = train_test_split(
    X, y, test_size=0.2, random_state=510, stratify=y
)
print(f"Training samples: {X_train_main.shape[0]}, Test samples: {X_test_main.shape[0]}")

# Resampling utilities
def apply_smoteenn(X, y, random_state=510):
...
def apply_resampling(X, y, method, random_state=510):
    if method == 'none':
        return X, y
    elif method == 'smote':
...
class FireMETA_DES:
    def __init__(self, k=9, dsel_ratio=0.3, acc_threshold=0.6, voting='soft', random_state=510):
        self.k = k
    ...
    def _build_pool(self):
        ...
        dt = DecisionTreeClassifier(max_depth=5, class_weight='balanced', random_state=self.random_state)
        rf = RandomForestClassifier(n_estimators=100, max_depth=6, class_weight='balanced',
                                    random_state=self.random_state, n_jobs=-1)
        xgb1 = xgb.XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.1,
                                 random_state=self.random_state, use_label_encoder=False, eval_metric='logloss')
        xgb2 = xgb.XGBClassifier(n_estimators=80, max_depth=5, learning_rate=0.05,
                                 random_state=self.random_state, use_label_encoder=False, eval_metric='logloss')
        lgbm = lgb.LGBMClassifier(n_estimators=100, max_depth=4, learning_rate=0.1,
                                  random_state=self.random_state, verbose=-1)
        cat = CatBoostClassifier(iterations=100, depth=4, learning_rate=0.1,
                                 random_seed=self.random_state, verbose=False)
        hgb = HistGradientBoostingClassifier(max_iter=100, max_depth=4, random_state=self.random_state)
...
# META-DES builder
def build_meta_des(X_base, y_base, X_dsel, y_dsel, method, random_state=510):
    ...
    hgb = HistGradientBoostingClassifier(max_iter=100, max_depth=4, random_state=random_state)
...
# Classifier factory
def get_classifier(clf_name, method, random_state=510, pos_scale=None):
    if clf_name == 'DT':
...
# 10x5 cross-validation on training set
n_repeats = 10
n_splits = 5
rskf = RepeatedStratifiedKFold(n_splits=n_splits, n_repeats=n_repeats, random_state=510)
...
for method in balance_methods:
    ...
    for clf_name in classifier_names:
        ...
        for train_idx, val_idx in rskf.split(X_train_main, y_train_main):
            X_train_fold = X_train_main.iloc[train_idx].values
            y_train_fold = y_train_main.iloc[train_idx].values
            X_val_fold = X_train_main.iloc[val_idx].values
            y_val_fold = y_train_main.iloc[val_idx].values
            ...
            if clf_name == 'META-DES':
                X_base, X_dsel, y_base, y_dsel = train_test_split(
                    X_res, y_res, test_size=0.3, stratify=y_res, random_state=510
                )
            else:
            ...
            if clf_name == 'DT':
                clf = get_classifier('DT', method, random_state=510)
                clf.fit(X_base, y_base)
                y_pred = clf.predict(X_val_scaled)
                y_proba = clf.predict_proba(X_val_scaled)[:, 1]
            elif clf_name == 'XGB':
                clf = get_classifier('XGB', method, random_state=510, pos_scale=scale_pos)
                clf.fit(X_base, y_base)
                y_pred = clf.predict(X_val_scaled)
                y_proba = clf.predict_proba(X_val_scaled)[:, 1]
            elif clf_name == 'META-DES':
                meta = build_meta_des(X_base, y_base, X_dsel, y_dsel, method, random_state=510)
                y_pred = meta.predict(X_val_scaled)
                y_proba = meta.predict_proba(X_val_scaled)[:, 1]
            elif clf_name == 'FIRE-META-DES':
                fire = FireMETA_DES(
                    k=9, dsel_ratio=0.3, acc_threshold=0.6, voting='soft', random_state=510
                )
